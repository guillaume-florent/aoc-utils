...
RUN apt-get update && apt-get install -y --no-install-recommends libgtk2.0-0 && rm -rf /var/lib/apt/lists/*
RUN conda install -y -c anaconda wxpython

RUN conda install -y pyqt
RUN apt-get update && apt-get install -y --no-install-recommends libgl1-mesa-dev libx11-xcb1 && rm -rf /var/lib/apt/lists/*
...
# Sometimes useful ...
RUN apt-get update && apt-get install -y --no-install-recommends gedit && rm -rf /var/lib/apt/lists/*